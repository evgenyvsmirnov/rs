...
import rs.vm.RSParser.CurveContext;
 ...
 *         ( from (460, 500) to (525, 430) towards (500, 400) rotate 75)
 *     )
 * </pre>
 */
public class CurvesInterpreter extends InterpreterBase implements ICodeBlockInterpreter<CurvesContext, ShapeResult>
{
    ...
    public ShapeResult interpret(CurvesContext ctx, DrawFrames frames, RSBaseVisitor<Object> visitor)
    {
        ...
        for (int i = 0; i < ctx.curve().size(); i++)
        {
            ...
            Point toPoint = evaluatePointFromExpressionOrRandom(curveContext.shapeCoord(1), visitor);
            Point towardsPoint = evaluatePointFromExpressionOrRandom(curveContext.shapeCoord(2), visitor);

            int rotate = evaluateRotationAngleFromExprOrRandom(curveContext.rotateShape(), visitor);
            ...
            shapes.add(new ShapeWithContext(
                    shape,
